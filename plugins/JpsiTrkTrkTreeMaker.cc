...
#include "JpsiTrkTrkTreeMaker.h"
         
/// default constructor
JpsiTrkTrkTreeMaker::JpsiTrkTrkTreeMaker(const edm::ParameterSet& cfg): 
  vertexes_           (cfg.getParameter<edm::InputTag>("vertexes")), 
  muons_              (cfg.getParameter<edm::InputTag>("muons")), 
  tracks_             (cfg.getParameter<edm::InputTag>("tracks")), 
  onlineTracks_       (cfg.getParameter<edm::InputTag>("TrkCandidatesLabel")), 
...
void JpsiTrkTrkTreeMaker::endJob() {
}


void JpsiTrkTrkTreeMaker::beginRun(const edm::Run & run, const edm::EventSetup & eventSetup) {
...
void JpsiTrkTrkTreeMaker::endRun(const edm::Run & run, const edm::EventSetup & eventSetup) {}
 
void JpsiTrkTrkTreeMaker::analyze(const edm::Event &event, const edm::EventSetup &eventSetup) {
...
//   event.getByLabel(onlineTracks_, onlineTracks);
  ...
  //get online beamspot position
//   edm::Handle<reco::BeamSpot> hltBeamSpotHandle;
//   event.getByLabel("hltOnlineBeamSpot",hltBeamSpotHandle);
//   const reco::BeamSpot& hltBeamSpot = *hltBeamSpotHandle;
  ...
  edm::Handle<reco::VertexCollection> hltTrkTrkVertex; 
  try   { event.getByLabel(MumuVtxProdTag_, hltTrkTrkVertex);}
...
//   const reco::VertexCollection & hltVertices = *hltVertexHandle.product();
  ...
  edm::Handle< reco::RecoChargedCandidateDoubleMap > d0Trk2Coll; 
  event.getByLabel(MumuVtxProdTag_.label(), "d0secondTrk", d0Trk2Coll);
  edm::Handle< reco::RecoChargedCandidatesDoubleMap > LSigmaColl; 
  event.getByLabel(edm::InputTag(MumuVtxProdTag_.label(), "LSigma"   , triggerProcess_), LSigmaColl);
  edm::Handle< reco::RecoChargedCandidatesDoubleMap > CosineColl; 
  event.getByLabel(edm::InputTag(MumuVtxProdTag_.label(), "Cosine"   , triggerProcess_), CosineColl);
  edm::Handle< reco::RecoChargedCandidatesDoubleMap > VertexColl; 
  event.getByLabel(edm::InputTag(MumuVtxProdTag_.label(), "VertexCL" , triggerProcess_), VertexColl);
  edm::Handle< reco::RecoChargedCandidatesDoubleMap > Comb1Coll; 
  event.getByLabel(edm::InputTag(MumuVtxProdTag_.label(), "Comb1Mass", triggerProcess_), Comb1Coll );
  edm::Handle< reco::RecoChargedCandidatesDoubleMap > Comb2Coll; 
  event.getByLabel(edm::InputTag(MumuVtxProdTag_.label(), "Comb2Mass", triggerProcess_), Comb2Coll );
      ...
      for(std::vector<Muon>::const_iterator mu2=mu1; mu2!=muons->end(); ++mu2) {
        ...
        if( muon::isLooseMuon( (*mu2)) && (*mu2).pt()>4 && fabs( (*mu2).eta())< 2.2) 
        {
          ...
          for (unsigned tracksIt =0 ;  tracksIt < tracks->size(); tracksIt++)
          {
            ...
            double minL3DeltaR = 0.5; 
            for(unsigned int il3=0; il3< (*d0Trk1Coll).size(); ++il3) {
              double tmpL3DeltaR = deltaR( itrk1.eta(), itrk1.phi(), (*d0Trk1Coll).at(il3).first->eta(),(*d0Trk1Coll).at(il3).first->phi()  ); 
            ...
            for (unsigned tracksIt2 = tracksIt + 1 ;  tracksIt2 < tracks->size(); tracksIt2++)
            {
              ...
              double minTrk2DeltaR = 0.5; 
              for(unsigned int il3=0; il3< (*d0Trk2Coll).size(); ++il3) {
                double tmpL3DeltaR = deltaR( itrk2.eta(), itrk2.phi(), (*d0Trk2Coll).at(il3).first->eta(),(*d0Trk2Coll).at(il3).first->phi()  ); 
                if(tmpL3DeltaR < minTrk2DeltaR) {
                  minTrk2DeltaR = tmpL3DeltaR; 
                  T_hlt_TrkMd0Sign = d0Trk2Coll->at(il3).second; 
                }
              }
              ...
              for (unsigned vtxIt = 0 ;  vtxIt < hltTrkTrkVertex->size(); vtxIt++){
                float idx = hltTrkTrkVertex->at(vtxIt).position().x() - tv.position().x();
                float idy = hltTrkTrkVertex->at(vtxIt).position().y() - tv.position().y();
                float idz = hltTrkTrkVertex->at(vtxIt).position().z() - tv.position().z();
                if (fabs(idx) < fabs(T_JpsiTkTkVtx_hltReco_dx)  &&  fabs(idy) < fabs(T_JpsiTkTkVtx_hltReco_dy)){
                  ...
                  T_hlt_JpsiTkTkPosition_x      = hltTrkTrkVertex->at(vtxIt).position().x();
                  T_hlt_JpsiTkTkPosition_y      = hltTrkTrkVertex->at(vtxIt).position().y();
                  T_hlt_JpsiTkTkPosition_z      = hltTrkTrkVertex->at(vtxIt).position().z();
                  T_hlt_JpsiTkTkPositionError_x = hltTrkTrkVertex->at(vtxIt).xError();
                  T_hlt_JpsiTkTkPositionError_y = hltTrkTrkVertex->at(vtxIt).yError();
                  T_hlt_JpsiTkTkPositionError_z = hltTrkTrkVertex->at(vtxIt).zError();
                  T_hlt_JpsiTkTkCL = TMath::Prob(hltTrkTrkVertex->at(vtxIt).chi2(), hltTrkTrkVertex->at(vtxIt).ndof() );
...
//                   hlt_secondaryVertexErr = hltVertices.at(vtxIt).positionError();
              ...
              hists_["pullX_4trks" ]->Fill( T_JpsiTkTkVtx_hltReco_dx/T_hlt_JpsiTkTkPositionError_x );
              hists_["pullY_4trks" ]->Fill( T_JpsiTkTkVtx_hltReco_dy/T_hlt_JpsiTkTkPositionError_y );
              hists_["pullZ_4trks" ]->Fill( T_JpsiTkTkVtx_hltReco_dz/T_hlt_JpsiTkTkPositionError_z );
              hists_["hltVtxProb"  ]->Fill( T_hlt_JpsiTkTkCL      );
              ...
              {
                if((LSigmaColl->at(il3).first.first->pt() == T_hlt_TrkPPt && LSigmaColl->at(il3).first.second->pt() == T_hlt_TrkMPt) ||
                   (LSigmaColl->at(il3).first.first->pt() == T_hlt_TrkMPt && LSigmaColl->at(il3).first.second->pt() == T_hlt_TrkPPt)
                  ) 
                { 
                  T_hlt_JpsiTkTkLSigma = LSigmaColl->at(il3).second; 
                  T_hlt_JpsiTkTkCosBS  = CosineColl->at(il3).second; 
                  T_hlt_Comb1Mass      = Comb1Coll ->at(il3).second; 
                  T_hlt_Comb2Mass      = Comb2Coll ->at(il3).second; 
                  break; 
                }
              }  
              ...
              {
                if((VertexColl->at(il3).first.first->pt() == T_hlt_TrkPPt && VertexColl->at(il3).first.second->pt() == T_hlt_TrkMPt) ||
                   (VertexColl->at(il3).first.first->pt() == T_hlt_TrkMPt && VertexColl->at(il3).first.second->pt() == T_hlt_TrkPPt)
                  ) 
                { 
                  T_hlt_JpsiTkTkCLTest = VertexColl->at(il3).second; 
                  break; 
                }
              }  
...
void JpsiTrkTrkTreeMaker::MonteCarloStudies(const edm::Event& iEvent)
{
...
void JpsiTrkTrkTreeMaker::beginJob() {
  ...
  outTree_->Branch("T_hlt_JpsiTkTkPosition_x"      , &T_hlt_JpsiTkTkPosition_x      );
  outTree_->Branch("T_hlt_JpsiTkTkPosition_y"      , &T_hlt_JpsiTkTkPosition_y      );
  outTree_->Branch("T_hlt_JpsiTkTkPosition_z"      , &T_hlt_JpsiTkTkPosition_z      );
  outTree_->Branch("T_hlt_JpsiTkTkPositionError_x" , &T_hlt_JpsiTkTkPositionError_x );
  outTree_->Branch("T_hlt_JpsiTkTkPositionError_y" , &T_hlt_JpsiTkTkPositionError_y );
  outTree_->Branch("T_hlt_JpsiTkTkPositionError_z" , &T_hlt_JpsiTkTkPositionError_z );
  ...
  outTree_->Branch("T_hlt_JpsiTkTkCL"              , &T_hlt_JpsiTkTkCL              );
  outTree_->Branch("T_hlt_JpsiTkTkCLTest"          , &T_hlt_JpsiTkTkCLTest          );
  outTree_->Branch("T_hlt_JpsiTkTkLSigma"          , &T_hlt_JpsiTkTkLSigma          );
  outTree_->Branch("T_hlt_JpsiTkTkCosBS"           , &T_hlt_JpsiTkTkCosBS           );
  outTree_->Branch("T_hlt_Comb1Mass"               , &T_hlt_Comb1Mass               );
  outTree_->Branch("T_hlt_Comb2Mass"               , &T_hlt_Comb2Mass               );
...
void JpsiTrkTrkTreeMaker::clearVar()
{
...
T_hlt_JpsiTkTkPosition_x      = 9999;      
T_hlt_JpsiTkTkPosition_y      = 9999;      
T_hlt_JpsiTkTkPosition_z      = 9999;      
T_hlt_JpsiTkTkPositionError_x = 9999;      
T_hlt_JpsiTkTkPositionError_y = 9999;      
T_hlt_JpsiTkTkPositionError_z = 9999;      
T_JpsiTkTkVtx_hltReco_dx      = 9999;      
T_JpsiTkTkVtx_hltReco_dy      = 9999;      
T_JpsiTkTkVtx_hltReco_dz      = 9999;      
T_hlt_JpsiTkTkCL              = 9999;      
T_hlt_JpsiTkTkCLTest          = 9999;     
T_hlt_JpsiTkTkLSigma          = 9999;      
T_hlt_JpsiTkTkCosBS           = 9999;      
...
DEFINE_FWK_MODULE(JpsiTrkTrkTreeMaker);
